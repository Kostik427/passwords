...
class EncryptionSettings:
    ...
    def get_settings(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT settings FROM encryption_settings ORDER BY id DESC LIMIT 1"
        )
    ...
    def update_settings(self, new_settings):
        ...
        if "iterations" in new_settings:
            if (
                not isinstance(new_settings["iterations"], int)
                or new_settings["iterations"] < 100000
            ):
                raise ValueError(
                    "Количество итераций должно быть целым числом >= 100000"
                )

        if "key_length" in new_settings:
            if (
                not isinstance(new_settings["key_length"], int)
                or new_settings["key_length"] < 16
            ):
        ...
        if "hash_algorithm" in new_settings:
            if new_settings["hash_algorithm"] not in self.HASH_ALGORITHMS:
                raise ValueError(
                    f"Поддерживаемые алгоритмы хеширования: {', '.join(self.HASH_ALGORITHMS.keys())}"
                )

        if "salt_size" in new_settings:
            if (
                not isinstance(new_settings["salt_size"], int)
                or new_settings["salt_size"] < 16
            ):
